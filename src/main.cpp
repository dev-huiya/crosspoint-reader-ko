...
void restoreLegacyKoLanguage() {
  // Only migrate cards that have no settings JSON. A failed JSON parse must
  // not silently overwrite an existing settings file.
  if (Storage.exists("/.crosspoint/settings.json")) return;
  HalFile file;
  if (!Storage.openFileForRead("I18N", "/.crosspoint/language.bin", file)) return;
  uint8_t bytes[2] = {};
  const bool valid = file.read(bytes, sizeof(bytes)) == sizeof(bytes);
  file.close();
  if (!valid) return;
  ...
  SETTINGS.saveToFile();  // Persist a stable code string for future enum changes.
}
}  // namespace
...
// Korean built-in fonts. Keep the upstream font cache and SD font infrastructure.
